...
server {{
    ...
    access_log /var/log/nginx/{DOMAIN_NAME_SAFE}_nginx.log combined;
...
acme_issue = shlex.join(
    [
        ...
        *itertools.chain(*[["-d", d] for d in server_name_list]),
        "--nginx",
        f"/etc/nginx/conf.d/{DOMAIN_NAME_SAFE}.conf",
        "--debug",
        "2",
    ]
)
...
acme_install = shlex.join(
    [
        ...
        *itertools.chain(*[["-d", d] for d in server_name_list]),
        "--key-file",
        f"/etc/nginx/ssl/{DOMAIN_NAME_SAFE}.key",
        "--fullchain-file",
        f"/etc/nginx/ssl/{DOMAIN_NAME_SAFE}.pem",
        "--reloadcmd",
